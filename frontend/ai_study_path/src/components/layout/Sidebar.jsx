    ...
    >
      ...
      <div className='flex items-center justify-between h-16 px-5 border-b border-slate-200/60'>
        <div className='flex items-center gap-3'>
          <div className='flex items-center justify-center w-9 h-9 rounded-xl bg-linear-to-br from-[#8686AC] to-[#434161] shadow-md shadow-[#a2a2cb]'>
            <BrainCircuit className='text-white' size={20} strokeWidth={2.5} />
          </div>
          <h1 className='text-sm md:text-base font-bold text-slate-900 tracking-tight'>AI Learning Assistance</h1>
        </div>
      ...
      <nav className='flex-1 px-3 py-6 space-y-1.5'>
        {navLinks.map((link) => {
          return (
          <NavLink key={link.to} to={link.to} onClick={toggleSidebar} className={({ isActive }) => 
          `group flex items-center gap-3 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-300
          ${isActive ? 'bg-linear-to-r from-[#8686AC] to-[#434161] text-white shadow-lg shadow-[#a2a2cb]' : 'text-slate-700 hover:bg-slate-100 hover:text-slate-900' }`}
          >
            {({ isActive }) => (
              <>
      ...
      <div className='px-3 py-4 border-t border-slate-200/20'>
        <button onClick={handleLogout} className='group flex items-center gap-3 w-full px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-red-50 hover:text-red-600 rounded-xl transition-all duration-200'>
          <LogOut size={18} strokeWidth={2.5} className='transition-all duration-200 group-hover:scale-110' /> Logout
        </button>
      </div>
    </aside>
  </>
