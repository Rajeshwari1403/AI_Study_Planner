    ...
    <div className='bg-white border border-neutral-200 rounded-lg p-6'>
      ...
      <Modal isOpen={isGenerateModalOpen} onClose={() => setIsGenerateModalOpen(false)} title="Generate New Quiz">
        <form onSubmit={handleGenerateQuiz} className='space-y-4'>
          <div>
            ...
            <input type='number' value={numQuestions} onChange={(e) => setNumQuestions(Math.max(1, parseInt(e.target.value) || 1))} min="1" required className='w-full h-9 px-3 border border-neutral-200 rounded-lg bg-white text-sm text-neutral-900 placeholder-neutral-400 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-blue-950 focus:border-transparent' />
          </div>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Confirm Delete Quiz">
        <div className='space-y-4'>
          <p className='text-sm text-neutral-600'>Are you sure you want to delete the quiz: <span className='font-semibold text-neutral-700'>{selectedQuiz?.title || 'this quiz'}</span>? This action cannot be undo!!!</p>
          <div className='flex justify-end gap-2 pt-2'>
            <Button type='button' variant='outline' onClick={() => setIsDeleteModalOpen(false)} disabled={deleting}>
              Cancel
            </Button>
            <Button onClick={handleConfirmDelete} disabled={deleting} className='bg-red-500 hover:bg-red-700 active:bg-red-600 focus:ring-red-500'>
