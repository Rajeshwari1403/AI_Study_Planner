...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.message || err.message || 'Failed to register. Please try again!';
      setError(errMsg);
      toast.error(errMsg);
    } finally {
      setLoading(false);
        ...
        <div className='flex items-center justify-center min-h-screen bg-linear-to-br from-slate-400 via-white to-slate-500'>
          ...
          <div className='space-y-5'>
            ...
            <div className='space-y-2'>
              ...
              <div className='relative group'>
                <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors duration-200
                  ${focusedField === "username" ? 'text-[#434161]' : 'text-slate-400' }`}
                >
            ...
            <div className='space-y-2'>
              <label className='block text-xs font-semibold text-slate-700 uppercase tracking-wide'>Password</label>
                <div className='relative group'>
                  <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors duration-200
                    ${focusedField === 'password' ? 'text-slate-500' : 'text-slate-800' }`}
                  >
          ...
          <div className='mt-8 pt-6 border-t border-slate-200/60'>
            <p className='text-center text-sm text-slate-600'>
              ...
              <Link to='/login' className='font-semibold text-[#343066] hover:text-[#201c4f] transition-colors duration-200'>Sign in</Link>
            </p>
          </div>
          </div>
